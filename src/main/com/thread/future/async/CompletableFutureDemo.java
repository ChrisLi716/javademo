package com.thread.future.async;
...
public class CompletableFutureDemo {
    ...
    public static void testCompletableFuture() {
        final List<CompletableFuture<String>> collect = IntStream.rangeClosed(1, 10)
                ...
                .map(future -> future.thenCompose(isDigits -> CompletableFuture.supplyAsync(() -> ToStrJson.toJson(isDigits))))
                .collect(Collectors.toList());

        //等待执行完毕
        List<String> collectStrList = collect.stream().map(CompletableFuture::join).collect(Collectors.toList());
    }
